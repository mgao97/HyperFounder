    ...
    def compute_structural_confidence(
        ...
        for i in range(num_nodes):
            ...
            if incident_edges.numel() > 1:
                # Count shared nodes across incident edges
                edge_nodes = [set(dense_inc[:, e].nonzero(as_tuple=True)[0].tolist()) for e in incident_edges]
                shared_count = 0
                for j, s1 in enumerate(edge_nodes):
                    for s2 in edge_nodes[j + 1:]:
                        shared_count += len(s1 & s2)
                overlap_score[i] = shared_count / max(len(edge_nodes) * (len(edge_nodes) - 1) // 2, 1)
            else:
                overlap_score[i] = 0.0
...
class EdgeConfidenceScorer(nn.Module):
    ...
    def compute_structural_confidence(
        ...
        for e_idx in range(num_edges):
            # Cardinality score: edge should have reasonable size
            members = dense_inc[:, e_idx].nonzero(as_tuple=True)[0]
            cardinality = len(members)
            
            # Ideal cardinality is 3-10
            if 3 <= cardinality <= 10:
                cardinality_score = 1.0
            elif 2 <= cardinality < 3 or 10 < cardinality <= 20:
                cardinality_score = 0.7
            else:
                cardinality_score = 0.3

            # Coherence score: member nodes should have similar structure
            if cardinality >= 2:
                member_embs = edge_emb[e_idx:e_idx+1].expand(cardinality, -1)
                coherence = 0.5  # Simplified
            else:
                coherence = 0.0

            # Overlap score: edge should share nodes with other edges
            edge_nodes = set(members.tolist())
            overlap_count = 0
            for other_idx in range(num_edges):
                if other_idx != e_idx:
                    other_members = dense_inc[:, other_idx].nonzero(as_tuple=True)[0]
                    if edge_nodes & set(other_members.tolist()):
                        overlap_count += 1
            overlap_score = overlap_count / max(num_edges - 1, 1)

            # Validity score
            validity_score = 1.0 if cardinality >= 2 else 0.0
