...
for i in trange(args.runs, desc='Run Train'):
    ...
    # 保存到文件
    np.savetxt('res/lahgnn_predicted_cocitationcora_lr.txt', predicted_array, fmt='%d')
...
# 从文件中读取预测结果
predicted_array = np.loadtxt('res/lahgnn_predicted_cocitationcora_lr.txt', dtype=int)
# 将numpy数组转换为列表
...
plt.savefig('figs/lahgnn_deg_acc_cocitationcora_lr.pdf')