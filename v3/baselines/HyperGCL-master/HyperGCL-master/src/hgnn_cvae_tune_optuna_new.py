...
def objective(trial):
    ...
    best_augmented_features, _ = hgnn_cvae_pretrain_new.get_augmented_features(args, hg, X, labels, idx_train, features_normalized, device)
    # best_augmented_features = torch.load('best_features.pt')
    print('optuna function get best augmented feats\n')
    print(best_augmented_features, best_augmented_features.shape)
    best_augmented_features = hgnn_cvae_pretrain_new.feature_tensor_normalize(best_augmented_features).detach()
