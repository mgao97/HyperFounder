    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                torch.save(net.state_dict(), 'model/hgnn_coauthorcora_best_model_1215.pth')
        scheduler.step()
    print("\ntrain finished!")
...
all_acc, all_microf1, all_macrof1 = [],[],[]
for run in range(5):
    net = HyperGCN(X.shape[1], 32, data["num_classes"])
    optimizer = optim.Adam(net.parameters(), lr=0.005, weight_decay=5e-4)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                best_val = val_acc
                best_state = deepcopy(net.state_dict())
                torch.save(net.state_dict(), 'model/hypergcn_coauthorcora_best_model_1215.pth')
        # scheduler.step()
    print("\ntrain finished!")
    print(f"best val: {best_val:.5f}")
...
for run in range(5):
    ...
    model_unigin = UniGIN(X.shape[1], 32, data["num_classes"], use_bn=True)
    optimizer = optim.Adam(model_unigin.parameters(), lr=0.005, weight_decay=5e-4)
    # scheduler = StepLR(optimizer, step_size=int(num_epochs/5), gamma=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                torch.save(model_unigin.state_dict(), 'unigin_coauthorcora_best_model_1215.pth')
...
for run in range(5):
    ...
    model_unisage = UniSAGE(X.shape[1], 32, data["num_classes"], use_bn=True)
    optimizer = optim.Adam(model_unisage.parameters(), lr=0.005, weight_decay=5e-4)
    # scheduler = StepLR(optimizer, step_size=int(num_epochs/5), gamma=0.1)
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            if val_acc > best_val:
                ...
                torch.save(model_unisage.state_dict(), 'unisage_coauthorcora_best_model_1215.pth')
