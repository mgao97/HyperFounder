...
for i in trange(args.runs, desc='Run Train'):
    ...
    for epoch in range(args.epochs):
        ...
        if loss_val < best:
            ...
            torch.save(best_model.state_dict(), 'model/lahgnn_cocitationcora_best_model.pth')
...
net = LAHGCN(concat=args.concat+1,
                  in_channels=features.shape[1],
                  hid_channels=args.hidden,
                  num_classes=labels.max().item() + 1,
                  use_bn=False,
                  drop_rate=args.dropout)
net.load_state_dict(torch.load('model/lahgnn_cocitationcora_best_model.pth'))
X, lbls = X.to(device), labels.to(device)
hg = hg.to(device)
net = net.to(device)


lahgnn_emb = net(X,hg)
print(f'hgnn_emb: {lahgnn_emb}', f'hgnn_emb size: {lahgnn_emb.shape}')
...
plt.savefig("emb_figs/lahgnn_cocitationcora.pdf", bbox_inches="tight") # save embeddings if needed
plt.savefig("emb_figs/lahgnn_cocitationcora.png", bbox_inches="tight")
plt.show()