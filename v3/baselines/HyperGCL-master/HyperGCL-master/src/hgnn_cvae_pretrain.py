...
def get_augmented_features(args, hg, features, labels, idx_train, features_normalized, device):
    ...
    torch.save(cvae_model.state_dict(), "cvae_model_pre_1206.pth") # 整个训练过程结束后，保存与训练得到的CVAE模型和最佳增强特征
    torch.save(best_augmented_features,'cvae_model_best_features_1206.pt')
