...
#                 torch.save(net.state_dict(), 'model/hgnn_cocitationciteseer_best_model_1224.pth')
#         scheduler.step()
#     print("\ntrain finished!")
...
model.load_state_dict(torch.load('model/lahgnn_cocitationciteseer_best_model_1224.pth'))
X, lbls = X.to(device), lbls.to(device)
hg = G.to(device)
net = model.to(device)
