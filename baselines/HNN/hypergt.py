...
# Try to import torch_sparse, fall back to built-in sparse if not available
try:
    from torch_sparse import SparseTensor, matmul
    USE_TORCH_SPARSE = True
except (ImportError, OSError) as e:
    # OSError can occur when CUDA version mismatch
    USE_TORCH_SPARSE = False
    print(f"torch_sparse not available ({e}), using built-in sparse operations")
...
def softmax_kernel_transformation(data, is_query, projection_matrix=None, numerical_stabilizer=0.000001):
    ...
    # If no projection matrix, just apply softmax directly
    if projection_matrix is None:
        diag_data = torch.square(data)
        diag_data = torch.sum(diag_data, dim=len(data.shape) - 1)
        diag_data = torch.unsqueeze(diag_data, dim=len(data.shape) - 1)
        last_dims_t = len(data.shape) - 1
        attention_dims_t = len(data.shape) - 3
        if is_query:
            data = torch.exp(data - diag_data - torch.max(data, dim=last_dims_t, keepdim=True)[0]) + numerical_stabilizer
        else:
            data = torch.exp(data - diag_data - torch.max(torch.max(data, dim=last_dims_t, keepdim=True)[0],
...
class HyperGT(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, args,
        ...
        # lazy init
        self.he_sparse_encoder = None
        self.hte_sparse_encoder = None
...
def kernelized_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None, tau=0.25, return_weight=True):
    ...
    z_num = z_num.permute(1, 0, 2, 3)
    z_den = z_den.permute(1, 0, 2)
    z_den = torch.unsqueeze(z_den, len(z_den.shape))
    z_output = z_num / z_den

    if return_weight:
        start, end = edge_index
        query_end, key_start = query_prime[end], key_prime[start]
        edge_attn_num = torch.einsum("ebhm,ebhm->ebh", query_end, key_start)
        edge_attn_num = edge_attn_num.permute(1, 0, 2)
        attn_normalizer = denominator(query_prime, key_prime)
        edge_attn_dem = attn_normalizer[end]
        edge_attn_dem = edge_attn_dem.permute(1, 0, 2)
        A_weight = edge_attn_num / edge_attn_dem
...
def kernelized_gumbel_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None,
    ...
    z_num = z_num.permute(1, 0, 2, 3, 4)
    z_den = z_den.permute(1, 0, 2, 3)
    z_den = torch.unsqueeze(z_den, len(z_den.shape))
    z_output = torch.mean(z_num / z_den, dim=3)
...
def add_conv_relational_bias(x, edge_index, b, trans='sigmoid'):
    ...
    for i in range(x.shape[2]):
        ...
        if USE_TORCH_SPARSE:
            ...
            # Use built-in PyTorch sparse tensor
            n = x.shape[1]
            ...
            conv_output.append(torch.sparse.mm(sparse_adj, x[:, :, i]))
    conv_output = torch.stack(conv_output, dim=2)
    return conv_output
...
class NodeFormerConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_heads, kernel_transformation=softmax_kernel_transformation,
        ...
        self.projection_matrix_type = None  # Not used, keep for compatibility
    ...
    def forward(self, args, z, adjs, tau):
        ...
        if self.use_gumbel and self.training:
            if self.use_edge_loss:
                z_next, weight = kernelized_gumbel_softmax(query, key, value, self.kernel_transformation,
                                                    projection_matrix, adjs[0],
                                                    self.nb_gumbel_sample, tau, self.use_edge_loss)
            else:
                z_next = kernelized_gumbel_softmax(query, key, value, self.kernel_transformation,
                                                   projection_matrix, adjs[0],
                                                   self.nb_gumbel_sample, tau, self.use_edge_loss)
        else:
            if self.use_edge_loss:
                z_next, weight = kernelized_softmax(query, key, value, self.kernel_transformation,
                                                    projection_matrix, adjs[0],
