...
class DPHGNN(nn.Module):
    def __init__(self, num_features, num_targets, args):
        ...
        self.fc = nn.Linear(
            in_features=self.spectral_layer.out_channels + self.taa_layer.out_channels, 
...
class DPHGNNConv(nn.Module):
    def __init__(self, in_channels, out_channels, args, bias=True, drop_rate=0.5, is_last=False):
    ...
    def forward(self, X, hyperedge_index, S_features):
        ...
        v2e_atten_score = F.dropout(
            self.atten_act(v2e_atten_score).squeeze(), self.dropout, training=self.training
        )
...
class TAAMSG(nn.Module):
    ...
    def forward(self, data):
        ...
        star_graph_features = self.sg_layer(x_star, data.A_sg)

        X_star, S_star = star_graph_features[data.v_mask], star_graph_features[~data.v_mask]
        
        '''
        2. Feature Mixture Module
        '''
        X_hyp, X_cg, X_sg = self.spectral_net(
            hypergcn_graph_features, clique_graph_features, star_graph_features, 
            data.L_hyp, data.L_cg, data.L_sg
        )
        X_spectral = self.taa_spectral_layer(X_hyp, X_cg, X_sg[data.v_mask])
        
        X_spatial = self.taa_spatial_layer(hypergcn_graph_features, clique_graph_features, X_star)
        
        taa_features = torch.cat((X_spatial, X_spectral), 1)
...
class HGSpectralNet(nn.Module):
    def __init__(self, in_channels, out_channels, bias=True, drop_rate=0.3):
...
class ExpanNet(nn.Module):
    def __init__(self, in_channels, out_channels, args, var='CG'):
        ...
        if self.num_layers == 1:
            ...
            self.convs.append(GCNConv(self.hidden_dim, out_channels))
