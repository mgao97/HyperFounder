...
from v1.models.shared_private_module import (
...
class TaskRouter(nn.Module):
    ...
    def forward(self, num_nodes: int, num_edges: int, incidence: torch.Tensor) -> torch.Tensor:
    ...
    def get_task_weights(self, num_nodes: int, num_edges: int, incidence: torch.Tensor) -> List[float]:
        with torch.no_grad():
            weights = self.forward(num_nodes, num_edges, incidence)
        return weights.cpu().tolist()


class TaskHeadsNegSamV1(nn.Module):
    """
    v1 heads: fixes the fatal bug where structural descriptors fed to the
    multi-granularity alignment were randomly sampled (torch.randn) instead of
    computed from real graph structure. Confidence routing now actually sees
    structure, so selective alignment is meaningful.
    """
    ...
    def __init__(
        ...
        self.node_disentangler = NodeDisentangler(in_dim=hidden_dim, shared_dim=s_dim, private_dim=p_dim)
        self.edge_disentangler = EdgeDisentangler(in_dim=hidden_dim, shared_dim=s_dim, private_dim=p_dim)
        self.private_domain_predictor = PrivateDomainPredictor(in_dim=p_dim, num_domains=num_domains)
    ...
    def compute_disentanglement_losses(
        ...
        node_losses = self.disentanglement_losses(
            z_node_shared, z_node_private, domain_labels, self.private_domain_predictor,
        )
        ...
        edge_losses = self.disentanglement_losses(
            z_edge_shared, z_edge_private, edge_domain_labels, self.private_domain_predictor,
        )
    ...
    def compute_alignment_losses(
        ...
        """
        v1 fix: structural descriptors are computed from REAL graph structure
        (degree / cardinality / overlap) instead of random noise, so the
        confidence router and prototype alignment receive meaningful signal.
        """
        losses = {}
        ...
        if node_descriptors is None and incidence is not None:
            node_descriptors = compute_structural_descriptors(node_emb, incidence, dim=4)
        if edge_descriptors is None and incidence is not None:
            edge_descriptors = compute_structural_descriptors(edge_emb, incidence, dim=4)

        if node_descriptors is None:
            node_descriptors = torch.zeros(node_emb.size(0), 4, device=node_emb.device)
        if edge_descriptors is None:
            edge_descriptors = torch.zeros(edge_emb.size(0), 4, device=edge_emb.device)
