...
from v1.models.encoder_v1 import UnifiedHypergraphEncoderV1
...
class FinetuneTrainerV1(DownstreamTrainerBase):
    ...
    def _run_node_task(self, encoder, graph, heads=None) -> Dict[str, float]:
        ...
        # Respect config's use_fp16; default to fp32. Forcing bf16 AMP (the
        # previous behaviour) silently collapses training on small _cc graphs
        # (val_acc freezes after epoch 1). Only enable AMP when explicitly asked.
        use_amp = self.device.type == "cuda" and bool(self.config["training"].get("use_fp16", False))
    ...
    def load_pretrained_heads(self):
        ...
        """
        from v1.models.heads_neg_sam_v1 import TaskHeadsNegSamV1
