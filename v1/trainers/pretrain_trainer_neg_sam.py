...
from v1.models.pretext_tasks_neg_sam import compute_pretraining_losses
...
class PretrainTrainerNegSam(TrainerBase):
    ...
    def _save_checkpoint(self, filename: str, epoch: int = 0) -> str:
    ...
    def train(self) -> Dict:
        ...
        bad_epochs = 0
        start_epoch = 0
        # === Resume support: load weights + optimizer from a previous checkpoint
        # and continue training up to `epochs` (interpreted as target total epochs). ===
        resume_ckpt = self.config["training"].get("resume_checkpoint")
        if resume_ckpt and Path(resume_ckpt).is_file():
            self._log(f"Resuming from checkpoint: {resume_ckpt}")
            ...
            # Restore best-loss tracking so early stopping keeps the prior best.
            restored_best = self.config["training"].get("resume_best_loss")
            if isinstance(restored_best, (int, float)):
                best_loss = float(restored_best)
                best_epoch = start_epoch
            self._log(
                f"Resumed: start_epoch={start_epoch}, target_epochs={epochs}, "
                f"restored_best_loss={best_loss}"
            )
        ...
        epoch_iterator = range(start_epoch + 1, epochs + 1)
        epoch_bar = None
        ...
        checkpoint_path = self._save_checkpoint("pretrain_last_neg_sam.pt", epoch=epochs)
