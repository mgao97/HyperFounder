...
meta_file="${1:-outputs/pids/pretrain_latest.meta}"
...
step_csv="${output_dir}/logs/pretrain_step_losses.csv"
epoch_csv="${output_dir}/logs/pretrain_losses.csv"
summary_json="${output_dir}/results/pretrain_summary.json"
...
if [[ -f "$step_csv" ]]; then
  ...
  echo "No loss CSV found under outputs/logs/."
fi
...
if [[ -n "$log_file" && -f "$log_file" ]]; then
  ...
  grep -E "Epoch .* step|Epoch .* done|Training finished|New best checkpoint|Early stopping" "$log_file" | tail -n 5 || true
fi
